...
def job_status(job_id: str, db: Session = Depends(get_db)) -> JobResponse:
    ...
    dxf_url = f"/api/jobs/{job_id}/download/dxf" if job.output_dxf_object else None
    dwg_url = f"/api/jobs/{job_id}/download/dwg" if job.output_dwg_object else None
...
@router.get("/api/jobs/{job_id}/download/dxf")
def download_dxf(job_id: str, db: Session = Depends(get_db)):
    job = get_job(db, job_id)
    if job is None or not job.output_dxf_object:
        raise HTTPException(status_code=404, detail="DXF ikke funnet")
    return RedirectResponse(public_url(job.output_dxf_object))

@router.get("/api/jobs/{job_id}/download/dwg")
def download_dwg(job_id: str, db: Session = Depends(get_db)):
    job = get_job(db, job_id)
    if job is None or not job.output_dwg_object:
        raise HTTPException(status_code=404, detail="DWG ikke funnet")
    return RedirectResponse(public_url(job.output_dwg_object))
